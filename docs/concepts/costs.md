...
`budget` is an optional hard spending limit for one workload. Your account's
monthly spending limit also applies across workloads. Without `budget`, the
account limit still applies. You do not need to provide an expected runtime.
...
| Account spend cap | Shared monthly customer spending limit |
...
provide it. Review the workload budget or account limit before retrying.
