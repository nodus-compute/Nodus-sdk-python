...
```sh
claude mcp add --scope user --transport stdio nodus -- uvx --from 'nodus-compute[mcp]==0.7.1' nodus-mcp
```
...
```sh
codex mcp add nodus -- uvx --from 'nodus-compute[mcp]==0.7.1' nodus-mcp
```
...
```json
{
  "mcp": {
    "nodus": {
      "type": "local",
      "command": ["uvx", "--from", "nodus-compute[mcp]==0.7.1", "nodus-mcp"],
      "enabled": true
    }
  }
}
...
`["--from", "nodus-compute[mcp]==0.7.1", "nodus-mcp"]`.
