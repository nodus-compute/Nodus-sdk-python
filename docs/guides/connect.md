...
```sh
claude mcp add --scope user --transport stdio nodus -- uvx --from 'nodus-compute[mcp]==0.6.0' nodus-mcp
```
...
```json
{
  "mcp": {
    "nodus": {
      "type": "local",
      "command": ["uvx", "--from", "nodus-compute[mcp]==0.6.0", "nodus-mcp"],
      "enabled": true
    }
  }
}
...
`["--from", "nodus-compute[mcp]==0.6.0", "nodus-mcp"]`.
