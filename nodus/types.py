...
class InterruptTolerance(_WireEnum):
    ...
    Read-only: the control plane derives it from ``continuity`` and models no
    field a caller could set, so ``run()`` does not accept it.
    """
...
@dataclass
class Meter:
    """Settled and accruing customer cost at ``as_of``.

    Server aggregates are authoritative, not sums of optional components.
    Subscription and saved-file charges are account-only components.
    ...
    def from_dict(cls, d: dict[str, Any] | None) -> "Meter | None":
        ...
        return cls(
            ...
            storage_settled_usd=_num(d.get("storage_settled_usd")),
