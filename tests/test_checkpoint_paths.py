...
def test_workload_file_sends_selected_scope_on_the_wire(tmp_path, asynchronous):
    source = tmp_path / 'workload.toml'
    source.write_text('command=["python", "train.py"]\n[continuity]\n'
                      'checkpoint_paths=["state", "progress.json"]', encoding='utf-8')
    requests = []
    ...
    assert requests[0]['continuity']['checkpoint_paths'] == ['state', 'progress.json']
