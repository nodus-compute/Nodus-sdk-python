...
def test_the_whole_journey(client, plane):
    ...
    try:
        ...
        # Logs before the first checkpoint are a refusal, not an empty string.
        with pytest.raises(nodus.NotFoundError) as exc:
            client.logs(workload.id)
        assert "log" in str(exc.value).lower()
    finally:
        # Cancel is also the cleanup: a failed assertion must not leave a paid
        # run going on a live plane.
        workload.cancel()
