...
@pytest.mark.asyncio
async def test_optimization_reaches_submission_without_defaults(tier, asynchronous):
...
def test_stage_routing_override_remains_distinct():
    stage = {
        "id": "train",
        "source": {"command": ["python", "train.py"]},
        "requirements": Requirements(optimization="faster", disk_gb=64, vcpus=8),
    }
